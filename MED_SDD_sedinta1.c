...
typedef struct nod {
	...
	struct nod* next; // adresa urmatorului nod din lista 
}nod;

// inserare: inceput + final + interior 
nod* insereazaInceput(nod* cap, Student stud) {
	...
	nod* nou = (nod*)malloc(sizeof(nod)); // se retine adresa de memorie
...
void insereazaFinal(nod** cap, Student stud) {
