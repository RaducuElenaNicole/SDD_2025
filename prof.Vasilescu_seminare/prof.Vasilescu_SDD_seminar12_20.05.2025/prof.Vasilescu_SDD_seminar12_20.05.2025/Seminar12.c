...
void afisareExamen(Examen e) {
	printf("%d. Nota minima de promovare a examenului la materia %s: %.2f", e.id, e.materie, e.nota);
}
...
void afisareGraf(NodPrincipal* graf) {
	while (graf != NULL) {
		...
		while (vecini) {
			printf("   ");
			afisareExamen(vecini->info->info); // nodSecundar - nodPrincipal - informatia din nodul principal
			vecini = vecini->next;
			printf("\n");
		}
		printf("\n"); 
...
void main() {
	...
	// pargurgerea in adancime  => Stiva
	printf("\n\n----------------------------\n");
	printf("Parcurgere in adancime:");
	parcurgereInAdancime(graf, 1);


	// pargurgerea in latime => Coada
}